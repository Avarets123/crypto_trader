...
type TradeTracker struct {
	mu     sync.Mutex
	trades map[string]*MomentumTrade // symbol → сделка
}

// NewTradeTracker создаёт TradeTracker.
func NewTradeTracker() *TradeTracker {
	return &TradeTracker{
		trades: make(map[string]*MomentumTrade),
	}
}

// Add добавляет сделку.
func (t *TradeTracker) Add(trade *MomentumTrade) {
	t.mu.Lock()
...
func (t *TradeTracker) Get(symbol string) (*MomentumTrade, bool) {
	t.mu.Lock()
