...
// TradeTracker — потокобезопасное хранилище активных scalping-сделок.
type TradeTracker struct {
	mu     sync.Mutex
	trades map[string]*ScalpTrade // symbol → сделка
}

// NewTradeTracker создаёт TradeTracker.
func NewTradeTracker() *TradeTracker {
	return &TradeTracker{
		trades: make(map[string]*ScalpTrade),
	}
}

// Add добавляет сделку.
func (t *TradeTracker) Add(trade *ScalpTrade) {
	t.mu.Lock()
...
func (t *TradeTracker) Get(symbol string) (*ScalpTrade, bool) {
	t.mu.Lock()
