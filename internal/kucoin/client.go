...
// Client управляет соединениями к KuCoin WebSocket.
type Client struct {
	config       *Config
	logger       *zap.Logger
	stats        *stats.Stats
...
func NewClient(cfg *Config, log *zap.Logger, st *stats.Stats, svc *ticker.TickerService) *Client {
	return &Client{
		config:  cfg,
		logger:  log,
		stats:   st,
...
func (c *Client) onSymbolsChanged(added, removed, all []string) {
	...
	c.logger.Info("kucoin: restarting connections",
...
func (c *Client) startConnections(symbols []string) (context.CancelFunc, *sync.WaitGroup) {
	...
	c.logger.Info("kucoin: starting connections",
		zap.Int("total_symbols", len(symbols)),
		zap.Int("connections", len(chunks)),
	)

	for i, chunk := range chunks {
		conn := NewConnection(i, chunk, c.config.RestURL, c.logger, c, c.config.MaxWait, c.stats)
		wg.Add(1)
